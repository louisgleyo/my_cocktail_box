...
include_once('./FonctionsRecettes.php');
...
function creationMenu()
{
     ...
     */
    if(!empty($Hierarchie[$dernier_aliment]['sous-categorie'])) {

        /*
         * On génère le code HTML du formulaire pour sélectionner une sous-catégorie.
         * On utilise la méthode GET et on est renvoyé sur la même page "Rechercher.php".
         * Le formulaire contient simplement un <select> par sous-catégorie.
         */
        echo "<form name=\"formulaire\" method=\"get\" action=\"Explorer.php\">
                    <ul class=\"actions\">
                        <li>
                            <select id=\"arborescence\" name=\"arborescence\" style=\"border-radius: 2.5em;height: 2.85em\">";
         ...
         */
        foreach ($Hierarchie[$dernier_aliment]['sous-categorie'] as $key => $value) {
...
function getUrlAliment($nom_aliment){
     ...
     */
    $debut_arborescence = substr($_GET['arborescence'],0,strpos($_GET['arborescence'],"|".$nom_aliment));
    /*
     ...
     * de l'arborescence.
     */
    return "Explorer.php?arborescence=".$url;
}
...
<html lang="en">
<head>
    <title>Explorer les recettes | MyCocktailBox</title>

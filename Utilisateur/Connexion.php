...
if(isset($_POST['submit'])) {
    ...
    if(1 == mysqli_num_rows($resultat)){ // le compte a été trouvé
        ...
        if(password_verify($_POST['mdp'],$hash)) {
            // definition de la variable de session utilisé pour verifier la connexion
            ...
            foreach ($_SESSION['favorites'] as $key => $recette) {
                ...
                $resultat = query($mysqli,
                    ...
                    "' AND boisson='" . $recette . "'");
                if (0 == mysqli_num_rows($resultat)) { // si elle ne vait pas deja partie des favorites
                    // ajout de la recette
                    query($mysqli,
            ...
            header('Location: ../Panier/MesRecettes.php');
            exit();
        }
    }
}
...
<head>
    <title>Se connecter | MyCocktailBox</title>
            ...
            <table class="alt">
                ...
                <tr><td></td>
                    <td><input type="submit" value="Valider" name="submit" /> &nbsp &nbsp
                        <a href="CreerCompte.php">Pas encore inscrit ?</a></td></tr>
